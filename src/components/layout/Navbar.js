  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-ink-900">
      <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" aria-label="Heavy Bazar — home" className="flex items-center">
          <Logo variant="onDark" priority className="h-5 w-auto sm:h-6" />
        </Link>
          ...
          {loading ? (
            ...
            <>
              <Link
                ...
                className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-white/5 hover:text-brand-400"
              ...
              <Link href={sellHref} className="ml-1.5">
                <Button variant="premiumOnDark" size="sm">
                  {isSeller ? <StoreIcon className="h-4 w-4" /> : null}
                  {sellLabel}
                </Button>
              </Link>

              <div className="relative ml-2 border-l border-white/10 pl-2.5" ref={menuRef}>
                <button
                  type="button"
                  onClick={() => setMenuOpen((v) => !v)}
                  className="flex items-center gap-2 rounded-lg py-1.5 pl-1.5 pr-2.5 text-sm font-medium text-slate-200 transition-colors hover:bg-white/5"
                >
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-xs font-bold text-ink-900 ring-1 ring-white/10">
                    {user.fullName?.charAt(0)?.toUpperCase() || <UserIcon className="h-4 w-4" />}
                  </span>
                  <span className="max-w-[9rem] truncate">{user.fullName}</span>
                  <ChevronDownIcon className={`h-4 w-4 text-slate-500 transition-transform ${menuOpen ? 'rotate-180' : ''}`} />
                </button>

                {menuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-60 overflow-hidden rounded-2xl border border-slate-200 bg-surface py-1.5 shadow-xl shadow-black/10">
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-300 hover:bg-white/5 hover:text-white lg:hidden"
